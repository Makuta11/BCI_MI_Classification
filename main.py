...
torch.save(model.state_dict(), 'outputs/SEQSEQModel_' + str(len(CONV_FILTERS)) + 'lay_seq' + str(SEQ_LENGTH) + 
...
with open('modelSaves/ModelOutputSEQSEQ_' + str(len(CONV_FILTERS)) + 'lay_seq' + str(SEQ_LENGTH) + '_batch' + str(BATCH_SIZE) + '_epoch' + str(NUM_EPOCHS) + '_Lr' + str(LEARNING_RATE) + 'LSTM2_8lay.pickle', 'wb') as handle:
